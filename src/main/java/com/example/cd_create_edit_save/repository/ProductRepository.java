...
public interface ProductRepository extends JpaRepository<Product, String> {
    ...
    ORDER BY ps.PRD_SHT_CD
   OFFSET CAST(COALESCE(:offset, 0) AS INT) ROWS
   FETCH NEXT CAST(COALESCE(:limit, 1000) AS INT) ROWS ONLY
""", nativeQuery = true)
    ...
    @Query(value = """
        ...
        LEFT JOIN tbl_product_sht_code ps 
            ON p.product_sht_cd = ps.prd_sht_cd
        WHERE (
            COALESCE(:text, '') = ''
            ...
            ORDER BY ps.PRD_SHT_CD
            OFFSET CAST(COALESCE(:offset, 0) AS INT) ROWS
            FETCH NEXT CAST(COALESCE(:limit, 100) AS INT) ROWS ONLY
        """,
            nativeQuery = true)
